...
func GetStudentsByGroup(ctx *gin.Context) {
	...
	ctx.IndentedJSON(200, gin.H{"students": students})
...
func AddStudent(ctx *gin.Context) {
	...
	if req.Group_id == nil {
		ctx.JSON(400, gin.H{"error": "введите группу для студента"})
		return
	}
	if req.Tuition_total == nil {
		ctx.JSON(400, gin.H{"error": "введите общую оплату для студента"})
		return
	}
	if *req.Tuition_total < 200000 {
		ctx.JSON(400, gin.H{"error": "минимальная общая оплата 200000!"})
		return
	}
	...
	ctx.JSON(201, gin.H{"created": student})
}
...
func PATCHStudent(ctx *gin.Context) {
	...
	var req struct {
		Tuition_paid *uint   `json:"tuition_paid"` // при изменении надо обязательно поменять сколько он заплатил
		Email        *string `json:"email"`
		Telegram     *string `json:"telegram"`
	...
	if req.Tuition_paid == nil {
		ctx.JSON(400, gin.H{"error": "оплатите хотябы рубль"})
		return
	}
	...
	ctx.JSON(200, gin.H{"edited": student})
}
...
func AddGroup(ctx *gin.Context) {
	...
	if req.Title == "" || len(req.Title) < 5 {
		ctx.JSON(400, gin.H{"error": "title пустой или короткий введите минимум 5 символов"})
		return
	}
	if req.Total_weeks != nil {
		if *req.Total_weeks <= 0 {
			ctx.JSON(400, gin.H{"error": "total_weeks не может быть отрицателен или равен нулю"})
			return
		}
		if *req.Total_weeks == 1 {
			ctx.JSON(400, gin.H{"error": "total_weeks должен быть хотя бы больше единицы"})
			return
		}
		...
		ctx.JSON(400, gin.H{"error": "обязательно введите total_weeks!"})
		return
	}
	...
	ctx.JSON(201, gin.H{"created": group})
}
...
func PATCHGroup(ctx *gin.Context) {
	...
	var req struct {
		Current_week uint `json:"current_week" binding:"required"`
	}
	...
	group, err := groups.UpdateGroup(uint(id), req.Current_week)
	if err != nil {
		ctx.JSON(400, gin.H{"error": err.Error()})
		return
	}

	ctx.JSON(200, gin.H{"edited": group})
...
func GetGroups(ctx *gin.Context) {
	...
	if week_s != "" {
		...
		ctx.IndentedJSON(200, gin.H{"groups": groups})
		return
	}
	if is_finished_s != "" {
		isFinished, err := strconv.ParseBool(is_finished_s)
		if err != nil {
			ctx.JSON(400, gin.H{"error": "используйте true или false для этого фильтра"})
			return
		}
...
func GetStudentByID(ctx *gin.Context) {
	...
	if id < 0 {
		ctx.JSON(400, gin.H{"error": "айди < 0!!!"})
		return
	}
